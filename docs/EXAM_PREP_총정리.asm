...
;
;  파일 구성:
;   [CH3-0]  교재 첫 번째 예제 — AddTwoProgram
;   [CH3-1]  프로그램 기본 구조 & 어셈블-링크 사이클
;   [CH3-2]  정수 리터럴 진법 / 상수 표현식(MOD) / 실수·문자·문자열 리터럴
;   [CH3-3]  데이터 타입 & 리틀 엔디안
;   [CH3-4]  문자열 정의 & DUP & 세 변수 합산 패턴 / .DATA?
;   [CH3-5]  심볼릭 상수 — = / $ / EQU / TEXTEQU
...
; ══════════════════════════════════════════════════════════════════
; [CH3-0]  교재 첫 번째 예제 — AddTwoProgram.asm
; ══════════════════════════════════════════════════════════════════
;  교재에서 처음 등장하는 최소 프로그램 구조 (5+6=11)
;
;  .386
;  .MODEL flat, stdcall
;  .STACK 4096
...
;  END main
;
;  ★ 세 변수 합산 패턴 (04_StringsAndArrays.asm) ★
;    x86은 메모리↔메모리 직접 연산 불가 → 레지스터 경유 필수
;
;  .DATA
;      val1 DWORD 10000h
;      val2 DWORD 40000h
...
; ══════════════════════════════════════════════════════════════════
; [CH3-2]  정수 리터럴 / 상수 표현식 / 실수·문자·문자열 리터럴
;          (02_IntegerLiterals.asm)
; ══════════════════════════════════════════════════════════════════
;
;  ★ 정수 리터럴 진법 표기 ★
;  ┌──────┬──────────┬──────────────────────────────────────────┐
...
;    expr3 DWORD  17 MOD 5         ; = 2  (나머지 연산)
;
;  실수 리터럴:
;    r4val  REAL4   3.14           ; 단정밀도 4바이트
;    r8val  REAL8   2.71828        ; 배정밀도 8바이트
;    r10val REAL10  1.0            ; 확장정밀도 10바이트
;    r4exp  REAL4   3.14E2         ; 지수 표기 = 314.0
;
;  문자 리터럴:
;    charA  BYTE  'A'              ; ASCII 41h = 65
;    charZ  BYTE  "Z"              ; 이중 따옴표도 허용 (5Ah = 90)
;
;  문자열 리터럴:
;    str1   BYTE  "ABCD"           ; 4바이트: 41h 42h 43h 44h
;    str2   BYTE  "ABCD", 0        ; 5바이트: 널 종료 문자열
;    str3   BYTE  "Hello", 0Dh, 0Ah, 0   ; CR(0Dh)+LF(0Ah) = 줄바꿈
;
;  ★ 줄 연속 문자 (\):  소스 두 줄을 하나의 정의로 합침
;    str4   BYTE  "First part ",\
;                 "second part", 0     ; \ 뒤에 공백/문자 없어야 함
;
;  여러 줄에 걸친 문자열 (레이블은 첫 줄에만):
;    longStr BYTE  "Line 1", 0Dh, 0Ah
;            BYTE  "Line 2", 0Dh, 0Ah
;            BYTE  "Line 3", 0


; ══════════════════════════════════════════════════════════════════
; [CH3-3]  데이터 타입 & 리틀 엔디안 ★ 시험 단골 ★
; ══════════════════════════════════════════════════════════════════
;
;  내장 데이터 타입 전체 표:
;  ┌──────────┬──────┬──────────────────────────────────────────┐
...
; ══════════════════════════════════════════════════════════════════
; [CH3-4]  문자열 / DUP / 세 변수 합산 / .DATA? / 코드-데이터 혼합
;          (04_StringsAndArrays.asm)
; ══════════════════════════════════════════════════════════════════
;
;  ★ CR+LF 문자열 패턴 ★
;    welcome BYTE "Welcome!", 0Dh, 0Ah, 0   ; 문자열 + 줄바꿈 + 널
;
;  ★ DUP 연산자: 횟수 DUP(초기값) ★
;    zeroBuf  BYTE  10  DUP(0)          ; 0이 10개 (10바이트)
;    undefBuf BYTE   5  DUP(?)          ; 미초기화 5바이트
;    pattern  BYTE   4  DUP(0ABh)       ; ABh가 4개
;    nested   BYTE   2  DUP(3 DUP(0FFh)); 중첩: FF×6 (2×3=6바이트)
;    wBuf     WORD   8  DUP(0)          ; WORD 8개 = 16바이트
;    dBuf     DWORD  4  DUP(?)          ; DWORD 4개 = 16바이트 미초기화
;
;  ★ 세 변수 합산 — 레지스터 경유 패턴 ★
;    (x86은 메모리↔메모리 직접 덧셈 불가, 레지스터를 중간에 사용)
;    val1 DWORD 10000h  /  val2 DWORD 40000h  /  val3 DWORD 20000h
;    → mov eax, val1   ; EAX = 10000h
;      add eax, val2   ; EAX = 50000h
;      add eax, val3   ; EAX = 70000h
;      mov sum, eax
...
; ══════════════════════════════════════════════════════════════════
; [CH3-5]  심볼릭 상수 — = / $ / EQU / TEXTEQU ★ 비교표 암기 ★
; ══════════════════════════════════════════════════════════════════
;
;  ┌────────────┬────────────────┬──────────┬──────────────────────┐
;  │ 디렉티브   │ 값의 종류      │ 재정의   │ 특징                 │
...
; ══════════════════════════════════════════════════════════════════
; [README 출제포인트]  각 장 README_중간고사출제포인트.asm 핵심 요약
; ══════════════════════════════════════════════════════════════════
;
; ── 3장 출제포인트 ────────────────────────────────────────────────
;  1. 리틀 엔디안
;     DWORD 12345678h → 주소 200h: [78h][56h][34h][12h]  (LSB 먼저)
;  2. = vs EQU vs TEXTEQU 재정의 여부 (= O / EQU X / TEXTEQU O)
;  3. $ 연산자
;     list  BYTE 10,20,30,40,50  →  ListSize = ($-list) = 5
;     wList WORD 1,2,3,4         →  wSize = ($-wList)/2 = 4
;  4. 16진수 리터럴: A~F 시작 시 앞에 0 필수 (FFh → 오류, 0FFh → 올바름)
;  5. DUP: BYTE 10 DUP(0) = 0이 10개 / BYTE 5 DUP(?) = 미초기화 5바이트
;  6. .DATA? : DUP(?) 와 함께 사용, 실행파일에 데이터 포함 안 됨
;  7. 디렉티브(BYTE DWORD .DATA .CODE PROC ENDP END) = 어셈블 시간
;     명령어(mov add sub nop jmp loop) = 런타임
;
; ── 4장 출제포인트 ────────────────────────────────────────────────
;  1. MOV 규칙: 메모리↔메모리 불가, 크기 일치 필수, EIP 목적지 불가
;  2. MOVZX vs MOVSX
;     BL=F0h: movzx eax,bl → 000000F0h(+240) / movsx eax,bl → FFFFFFF0h(-16)
;  3. CF vs OF — INC/DEC는 CF에 영향 없음!
;     AL=7Fh(127), add al,1 → AL=80h(-128): CF=0, OF=1
;     AL=FFh(255), add al,1 → AL=0:         CF=1, OF=0, ZF=1
;  4. TYPE / LENGTHOF / SIZEOF
;     arr DWORD 1,2,3,4,5 → TYPE=4  LENGTHOF=5  SIZEOF=20
;  5. PTR + 리틀 엔디안
;     myDouble DWORD 12345678h
;     WORD PTR myDouble → AX=5678h(하위 워드)
;     BYTE PTR myDouble → AL=78h(최하위 바이트)
;  6. LOOP 치명적 실수: ECX=0으로 시작 → FFFFFFFFh번(42억 회!) 반복
;  7. 배열 합산 7단계 (①OFFSET ②LENGTHOF ③초기화 ④레이블 ⑤add[esi] ⑥add TYPE ⑦loop)
;
; ── 5장 출제포인트 ────────────────────────────────────────────────
;  1. PUSH: ESP-=4, [ESP]=값  /  POP: 목적지=[ESP], ESP+=4
;     POP은 메모리를 지우지 않음 (ESP만 이동)
;     스택 성장 방향: 높은 주소 → 낮은 주소
;  2. CALL/RET (시험 단골!)
;     CALL: ESP-=4, [ESP]=복귀주소, EIP=함수시작주소
;     RET : EIP=[ESP], ESP+=4
;  3. PUSH/POP 균형 법칙: PUSH 횟수 = POP 횟수 (불균형 → 프로그램 충돌)
;  4. USES 주의: 반환값 레지스터(EAX)는 USES에 절대 넣지 않기!
;     (USES eax → ret 전에 pop eax 자동삽입 → 반환값 덮어써짐)
;  5. PUSHAD/POPAD: 8개 레지스터 한번에 / 반환값 EAX일 때 POPAD 금지
;  6. Irvine32 입출력:
;     WriteString → EDX=OFFSET 문자열  ← OFFSET 빠뜨리면 버그!
;     WriteInt    → EAX=부호있는 정수
;     WriteDec    → EAX=부호없는 정수
;     WriteChar   → AL=ASCII 코드
;     ReadString  → EDX=버퍼오프셋, ECX=버퍼크기 → EAX=입력수
;     ReadInt     → EAX=부호정수, 범위초과 OF=1
;     ReadDec     → EAX=무부호정수, 오류 CF=1
;     RandomRange → EAX(상한+1) 입력 → EAX=0~상한 난수
;  7. 스택 활용 4가지: ①레지스터보존 ②복귀주소 ③인수전달 ④지역변수
